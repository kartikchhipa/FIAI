...
model_path = "models/RRDB_ESRGAN_remote_train.pth"
orig_model_path = "models/RRDB_ESRGAN_x4.pth"
device = torch.device('cuda')  # if you want to run on CPU, change 'cuda' -> cpu
# device = torch.device('cpu')

# folder for low res and corresponding hi res image
test_LR_folder = 'input/remote_sensing/test/test_LR/*'
test_HR_folder = 'input/remote_sensing/test/test_HR/'
...
for path in glob.glob(test_LR_folder):
    ...
    plt.imsave('output/remote_sensing/{:s}_train.tiff'.format(base), output_img,cmap='gray')
...
for path in glob.glob(test_LR_folder):
    ...
    output_img = (output_img * 255.0).round()
    output_img = output_img.astype(np.uint8)
    plt.imsave('output/remote_sensing/{:s}_orig.tiff'.format(base), output_img, cmap='gray')
